...
def update(next_stateR, next_stateL):
	...
	if (cur_stateR != next_stateR):
		p1 = Process(target = motor1, args = (cur_stateR, cur_stateL, next_stateR, next_stateL))
		p1.start()

	if (cur_stateL != next_stateL):
		p2 = Process(target = motor2, args = (cur_stateR, cur_stateL, next_stateR, next_stateL))
		p2.start()

def motor1(cur_stateR, cur_stateL, next_stateR, next_stateL):
	#first set to 64 
	print 'start motor1'
	cur_stateR = M1S
	refresh(cur_stateR, cur_stateL)
	time.sleep(1)
	'''for x in range (cur_stateR, M1S):
		cur_stateR = x
		refresh(cur_stateR, cur_stateL)
		time.sleep(.01)
	for x in range (cur_stateR, M1S,-1):
		cur_stateR = x
		refresh(cur_stateR, cur_stateL)
		time.sleep(.01)'''
	...
	if next_stateR == M1F:
		...
		for x in range (M1S, M1B, -1):
			...
			time.sleep(.05)
			refresh(cur_stateR, cur_stateL)
	save_cur_state(cur_stateR, cur_stateL)
	print 'end motor1'



def motor2(cur_stateR, cur_stateL, next_stateR, next_stateL):
	#first set to 64
	print 'start motor2'
	cur_stateL = M2S
	refresh(cur_stateR, cur_stateL)
